...
<script type="text/javascript" src="${ctx }/plupload-2.3.1/plupload.full.min.js"></script>
<script>
...
function pldeleteFile(fileId){
	$.messager.confirm('确认', '是否确认删除此文件?', function(r){
		if(r){
			...
			$.ajax({
				type : 'post',
				url : "/notificationUpload/deleteFile",
				data : 'id=',
				dataType: "json",
...
$(function(){
	var uploader = new plupload.Uploader({
        ...
        url : "/notificationUpload/fileUpload?userId",
...
&nbsp;&nbsp;<span id="uploader" style="cursor:pointer;" class="uploader-btn">添加附件</span>
</div>
	...
	<table id="filelist" style="width:100%;" class="formtable">
		<c:forEach items="${attachmentList}" var="attachment" varStatus="i">
			<tr id="attachement${i.index }">
				...
				<td style="display:none">${attachment.id },${attachment.fileUrl },${attachment.fileSize },${attachment.fileName }</td>
				...
				<td style="display:none">${attachment.fileUrl},${attachment.creatorName},<fmt:formatDate value="${attachment.createTime}" type="both" pattern="yyyy-MM-dd HH:mm:ss"/></td>
			</tr>
		</c:forEach>
	</table>
